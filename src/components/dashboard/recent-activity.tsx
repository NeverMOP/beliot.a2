...
const getWarningDetails = (device: Device): { text: string; icon: React.ReactNode } => {
    ...
    <Card>
      ...
      <CardContent className="grid gap-2">
        {recentAlerts.length > 0 ? recentAlerts.map((device) => {
            const warningDetails = getWarningDetails(device);
            return (
              <Link href={`/devices/${device.id}`} key={device.id} className="block rounded-lg -mx-2 px-2 py-3 transition-colors hover:bg-muted">
